...
data_path_out = '/Users/maximoldenbeek/Dropbox/Thesis/datasets/FAUST_preprocesed/heat_patches'
data_path_out_smooth  = '/Users/maximoldenbeek/Dropbox/Thesis/datasets/FAUST_preprocesed/heat_patches_smooth'
...
for file_path in registrations:
    ...
    file_out_smooth = re.sub('\.ply','_all_conn.pt', file)
    
    path_out = os.path.join(data_path_out, file_out)
    path_out_smooth = os.path.join(data_path_out_smooth, file_out_smooth)
    ...
    adj_mat = adjacency_matrix_heat(mesh,p_max = 0.05)
    ...
    torch.save({'ind':i, 'data':v, 'size':s}, path_out_smooth)
    print('Succesful: {:.2f}'.format(time.time()-start))

    del mesh, adj_mat, mat